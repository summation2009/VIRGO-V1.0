...
void publishData(float dhtH, float dhtT, float rsH, float rsT,
                 float bmeH, float bmeT, float bmeP, float bmeG) {
  if (!client.connected()) {
    reconnect();
  }
  client.loop();
  ...
  sprintf(msg,
          ...
          "\"BME_Humi\":%.2f,\"BME_Temp\":%.2f,"
          "\"BME_Press\":%.2f,\"BME_Gas\":%.2f}",
          dhtH, dhtT, rsH, rsT,
          bmeH, bmeT, bmeP, bmeG);
...
if (now - lastPublish >= publishInterval) {
  ...
  if (isnan(h) || isnan(t)) {
    ...
    int32_t bmeTemp, bmeHumi, bmePress, bmeGas;
    BME680.getSensorData(bmeTemp, bmeHumi, bmePress, bmeGas);

    float tempC = bmeTemp / 100.0;
    float humiP = bmeHumi / 1000.0;
    float pressHpa = bmePress / 100.0;
    float gasKOhm = bmeGas / 100.0;

    publishData(h, t, Humi, Temp, humiP, tempC, pressHpa, gasKOhm);
  }
}
